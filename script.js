...
const TRANSLATIONS = {
  en: {
    ...
    "featured.badge":"FEATURED PROJECT","featured.summary":"End-to-end lakehouse platform with incremental processing, data quality, reliability testing, and measured performance tuning.","featured.quality":"Quality","featured.recovery":"Recovery","featured.monitoring":"Monitoring","featured.view":"View Project Details","featured.evidence":"Engineering Evidence",
    ...
    "projects.eyebrow":"FEATURED PROJECTS","projects.heading":"Completed Projects","projects.lead":"Production-style data engineering projects with real-world business context and verified evidence.","projects.viewAll":"View All Projects","projects.viewProject":"View Project","projects.privateRepo":"Private Repo • Verified Evidence","projects.fullHeading":"Engineering solutions to real data challenges.","projects.fullHeading.pre":"Engineering solutions to","projects.fullHeading.accent":"real data","projects.fullHeading.post":"challenges","projects.fullLead":"Six completed repositories. Each project begins with a real data problem and shows how the system was built, validated, operated, and explained.",
    ...
    "project.databricks.summary":"Medallion architecture with Delta Lake, CDC, SCD2, reliability labs, and measured Spark optimization.","project.databricks.detail":"A medallion lakehouse that demonstrates CDC, SCD Type 2, controlled failure labs, monitoring, troubleshooting, and measured Spark performance tuning.",
  ...
  th: {
    ...
    "featured.badge":"โปรเจกต์เด่น","featured.summary":"Lakehouse แบบ end-to-end ที่รองรับ incremental processing, data quality, reliability testing และ performance tuning พร้อมหลักฐานวัดผล","featured.quality":"คุณภาพข้อมูล","featured.recovery":"การกู้คืน","featured.monitoring":"การมอนิเตอร์","featured.view":"ดูรายละเอียดโปรเจกต์","featured.evidence":"ดูหลักฐานวิศวกรรม",
    ...
    "projects.eyebrow":"โปรเจกต์เด่น","projects.heading":"โปรเจกต์ที่ทำเสร็จแล้ว","projects.lead":"โปรเจกต์ Data Engineering แบบ production-style ที่เชื่อมกับบริบทธุรกิจจริงและมีหลักฐานรองรับชัดเจน","projects.viewAll":"ดูโปรเจกต์ทั้งหมด","projects.viewProject":"ดูโปรเจกต์","projects.privateRepo":"Private Repo • Verified Evidence","projects.fullHeading":"Engineering solutions to real data challenges.","projects.fullHeading.pre":"Engineering solutions to","projects.fullHeading.accent":"real data","projects.fullHeading.post":"challenges","projects.fullLead":"มีโปรเจกต์ที่ทำเสร็จแล้ว 6 โปรเจกต์ แต่ละโปรเจกต์เริ่มจากปัญหาข้อมูลจริง และแสดงหลักฐานตั้งแต่การสร้าง การตรวจสอบ การใช้งาน ไปจนถึงการอธิบายระบบอย่างเป็นขั้นตอน",
    "project.sql.summary":"แพลตฟอร์ม SQL ที่เน้น incremental load, data quality, monitoring, API serving และ recovery","project.sql.detail":"แพลตฟอร์ม SQL ที่เน้นความน่าเชื่อถือ รองรับ incremental loading, idempotency, quality/freshness gates, monitoring และ recovery พร้อม Airflow, read-only API, Docker และ CI","project.databricks.summary":"สถาปัตยกรรม Medallion พร้อม Delta Lake, CDC, SCD2, reliability labs และ Spark optimization ที่วัดผลได้","project.databricks.detail":"Lakehouse แบบ medallion ที่สาธิต CDC, SCD Type 2, controlled failure labs, monitoring, troubleshooting และการปรับจูนประสิทธิภาพ Spark ที่มีผลวัดรองรับ","project.influencer.summary":"Influencer Master Data ที่เชื่อม source provenance, campaign history และ explainable matching decision ให้ตรวจสอบย้อนกลับได้","project.influencer.detail":"โปรเจกต์ Master Data + PostgreSQL ที่ resolve identity จากข้อมูล Influencer ที่กระจัดกระจายเป็น Golden Master ที่ trace กลับ source ได้ เก็บ campaign/performance history และรองรับ explainable matching พร้อม human review","project.marketing.summary":"Pipeline ข้อมูล payment ที่มี validation, rejected-record routing, SQLite analytics และผลลัพธ์ที่ปลอดภัยต่อ Portfolio","project.marketing.detail":"Pipeline ข้อมูล payment ที่คำนึงถึง privacy โดยตรวจสอบข้อมูลธุรกิจ แยกรายการที่ไม่ผ่านการตรวจสอบ โหลดเข้า SQLite สร้าง analytical summaries และส่งออกผลลัพธ์สาธารณะที่ปลอดภัย 6 ชุด","project.ecommerce.summary":"Pipeline Excel หลายแหล่งที่มี validation, transformation, SQLite analytics และ portfolio-safe outputs","project.ecommerce.detail":"ETL pipeline จาก Excel หลายแหล่งที่จัดมาตรฐาน daily shop metrics ตรวจสอบ 105 records โหลดเข้า SQLite สร้าง SQL analytics และเผยแพร่ตัวอย่าง output ที่ปลอดภัย",
    "project.bigquery.summary":"แพลตฟอร์ม GCP/BigQuery แบบ production-oriented ที่มี layered warehouse, dbt, reconciliation, security, Terraform และ CI/CD","project.bigquery.detail":"แพลตฟอร์มข้อมูลบน Google Cloud แบบ production-oriented ครอบคลุม GCS landing, BigQuery แบบ RAW/STAGING/CORE/MART, dbt-bigquery, incremental และ idempotent processing, DQ/reconciliation, orchestration, IAM/security, governance, cost controls, Terraform, CI/CD และส่วนขยาย Financial Operations ที่ใช้ข้อมูล synthetic อย่างชัดเจน",
